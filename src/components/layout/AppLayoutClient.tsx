...
export const AppLayoutClient: React.FC<{ children: React.ReactNode }> = ({
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-satubg-light flex flex-col items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          <div className="w-10 h-10 rounded-2xl bg-satublue-50 border border-satublue-100 flex items-center justify-center">
  ...
  if (isSupabaseEnabled && !user && !isGuest) {
    return (
      <div className="min-h-screen bg-satubg-light flex flex-col selection:bg-satublue-100">
        <header className="w-full border-b border-slate-200/80 bg-white/80 backdrop-blur-md py-3.5 px-6">
  ...
  return (
    <div className="min-h-screen bg-satubg-light flex flex-col selection:bg-satublue-100 relative">
      {/* Gentle ambient aura in the backdrop */}
      <div className="fixed inset-0 -z-10 pointer-events-none overflow-hidden">
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[600px] sm:w-[900px] h-[350px] bg-gradient-to-b from-blue-50/50 via-indigo-50/20 to-transparent blur-3xl" />
      </div>
